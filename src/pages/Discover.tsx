...
export default function Discover() {
  ...
  const { data: cafes = [], isLoading } = useQuery({
    queryKey: ["cafes"],
    queryFn: async () => {
      const { data, error } = await supabase.from("cafes").select("id, slug, name, city, description")
    ...
    <div className="min-h-screen bg-gradient-hero">
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="mt-6 space-y-3">
          {isLoading ? <div className="grid place-items-center py-16"><Loader2 className="w-6 h-6 animate-spin text-muted-foreground" /></div>
