-- Update get_live_ops_board to only show orders that have a payment method selected
-- This hides "draft" orders where the customer is still in the PaymentDialog.
  ...
  SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.created_at), '[]'::jsonb) INTO _orders FROM (
    SELECT o.id, o.customer_name, o.table_no, o.source::text, o.status::text, o.payment_status::text, o.payment_method,
     ...
     WHERE o.cafe_id = _cafe_id
       AND o.payment_method IS NOT NULL -- FORCE: only show orders that have chosen a method
